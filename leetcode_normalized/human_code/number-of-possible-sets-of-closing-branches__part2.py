class C1(object):

    def numberOfSets(self, a1, a2, a3):
        """
        """

        def check(a1, a2):
            return all((a2[i][j] <= a2 for v1 in range(a1) if a1 & 1 << v1 for v2 in range(v1 + 1, a1) if a1 & 1 << v2))

        def floydWarshall(a1, a2):
            for v1 in range(len(a2[0])):
                if a1 & 1 << v1 == 0:
                    continue
                for v2 in range(len(a2)):
                    if a1 & 1 << v2 == 0:
                        continue
                    for v3 in range(v2 + 1, len(a2[v2])):
                        if a1 & 1 << v3 == 0:
                            continue
                        a2[v3][v2] = a2[v2][v3] = min(a2[v2][v3], a2[v2][v1] + a2[v1][v3])
            return check(a1, a2)
        v1 = [[0 if u == v else float('inf') for v2 in range(a1)] for v3 in range(a1)]
        for v3, v2, v4 in a3:
            v1[v3][v2] = min(v1[v3][v2], v4)
            v1[v2][v3] = min(v1[v2][v3], v4)
        return sum((floydWarshall(mask, [d[:] for v5 in v1]) for v6 in range(1 << a1)))
